...
async def upload(e, s):
    ...
    try:
        lf=await asyncio.to_thread(bucket.start_large_file, file_name=obj, content_type="b2/x-auto")
        fid=lf.id_
        async for chunk in msg.client.iter_download(msg.media, request_size=PART_SIZE, chunk_size=PART_SIZE):
            ...
            while len(buf)>=PART_SIZE:
                ...
                await asyncio.to_thread(bucket.upload_part, file_id=fid, part_number=partno, data_bytes=part, sha1_sum=h)
        ...
        if buf:
            ...
            await asyncio.to_thread(bucket.upload_part, file_id=fid, part_number=partno, data_bytes=part, sha1_sum=h)
            sha.append(h); done+=len(part)
        await asyncio.to_thread(bucket.finish_large_file, file_id=fid, part_sha1_array=sha)
        await status.edit(f"✅ **UPLOAD COMPLETE**\n\n📄 `{name}`\n📁 `{folder or '(root)'}`\n\n☁️ **B2 path:**\n`{obj}`\n\n📦 Size: `{size(done)}`")
    except Exception as ex:
        if fid:
            try: await asyncio.to_thread(bucket.cancel_large_file, fid)
            except: pass
